...
const ruMapType = [
    "Границы", "Населенные пункты", "Терминалы", "Деревья", "Инфраструктура", "Пляжи", "Парковки", "Церкви", "Здания", "Железные дороги", "Дороги", "Реки"
];
...
themeButton.addEventListener("click", () => {
    ...
    themeButton.background = darkMode ? "fff" : "fff";
...
function updateIconButtonStyles() {
    const iconSize = 60; // Размер иконки
...
function updateMapMode() {
    ...
    updateButtonStyles();
    updateIconButtonStyles();
    darkMode ? loadDistrictMaps(colorSchemeNight) : loadDistrictMaps(colorSchemeDay);
    changeIconColor();
}
...
async function loadDistrictMaps(colorScheme) {
    showLoader("block");
    clearMap();
    let loaderColorCounter = mapType.length;
    for (const district of districts) {
        for (const type of mapType) {
            if (buttons[type].classList.contains(`${type}VisibleON`)) {
                ...
                data: cachedData[cacheKey],
                    ...
                    //console.log(`Карта загружена: ${district} - ${type}`);
                loader.innerHTML = `Загрузка карты ${district} - ${type}`;
                loader.style.backgroundColor = loaderColors[(loaderColorCounter / 12 % loaderColors.length) | 0];
...
function getClosestDistrict() {
    ...
    let detailFactor = zoomTransform.k > 10 ? 1 : 2; // На большом зуме точнее
    ...
    //requestIdleCallback(() => {
        ...
        if (mapData && zoomTransform.k < 30) {
            ...
            mapData.data.features.forEach(feature => {
                ...
                if (bbox[1][0] < visibleX || bbox[0][0] > visibleX + visibleWidth ||
                    bbox[1][1] < visibleY || bbox[0][1] > visibleY + visibleHeight) {
            ...
            canvas.style.backgroundColor = darkMode ? "#091329" : "#27543d"; // Цвет фона при зуме
        }
        ...
        if (zoomTransform.k > 10) {
            ...
            if (districtData[currentDistrict]) {
                districtData[currentDistrict].forEach(layer => {
                    ...
                    let isWater = layer.strokeColor === colorSchemeDay["Water"][1] || layer.strokeColor === colorSchemeNight["Water"][1];
                    let isRoad = layer.strokeColor === colorSchemeDay["Roads"][1] || layer.strokeColor === colorSchemeNight["Roads"][1];
                    let isBuilding = layer.strokeColor === colorSchemeDay["Buildings"][1] || layer.strokeColor === colorSchemeNight["Buildings"][1];
                    ...
                    layer.data.features.forEach(feature => {
                        ...
                        let lineWidth = isWater
                            ...
                            : isRoad ? Math.max((((feature.properties.maxspeed) / 20) + 1) / (zoomTransform.k + 4), 0.0002)
                            : Math.max(layer.lineWidth / (zoomTransform.k + 5), 0.0002);

                        ctx.lineWidth = lineWidth;
                        ctx.strokeStyle = layer.strokeColor;

                        if ((isWater || isRoad) && lineWidth <= 0.005 || feature.properties.width > 5 && zoomTransform.k > 50) {
                            ctx.stroke();
                            feature.properties.name && zoomTransform.k > 500 ? addRiverLabel(feature) : '';
                            }

                        if (isRoad && (lowSpeedRoad && zoomTransform.k > 250 || highSpeedRoad && zoomTransform.k > 50)) {
                            ctx.stroke();
                        }

                        

                        if (isBuilding && zoomTransform.k > 800){
                            ctx.stroke();
                            ctx.fillStyle = layer.fillColor;
                            ctx.fill();
                        }

                        if (layer.fillColor !== "none" && !isBuilding) {
                            ctx.stroke();
                            ctx.fillStyle = layer.fillColor;
                            ctx.fill();
                            }

                        if (layer.fillColor && layer.fillColor == "none" && !isWater && !isRoad)
                        {
                            ctx.stroke();
                        }
                    });
        ...
        if (hoveredFeature) { // Подсветка выделенного региона
            ...
            if (darkMode) {
                ...
                gradient.addColorStop(0, "#060826"); // Цвет для темной темы
            } else {
                gradient.addColorStop(0.7, "#EDC5AB");
                gradient.addColorStop(0, "#E7EAEF"); // Цвет для светлой темы
            }

            if (zoomTransform.k < 10) {
                for (let i = 5; i > 0; i--) { //Обводка региона
                    ctx.globalAlpha = 1 / i;
                    ctx.beginPath();
                    path(hoveredFeature);
                    ctx.lineWidth = 2 + i - zoomTransform.k / 10;
                    ctx.strokeStyle = darkMode ? `rgba(232, 171, 85, ${1 - i / 5})` : `rgba(18, 47, 71, ${i / 5})`;
                    ctx.stroke();
                }
                ctx.globalAlpha = Math.max((1 - zoomTransform.k / 10), 0);
            }
            if (zoomTransform.k >= 10 && zoomTransform.k <= 11 || zoomTransform.k >=100 && zoomTransform.k <=150) { 
                hoveredFeature = null;
            ...
            if (zoomTransform.k > 11) {
                ctx.fillStyle = darkMode ? (zoomTransform.k <= 100 ? gradient : "#53576e") : "#ffdfdd";
                ctx.fill();
                ctx.lineWidth = zoomTransform.k < 100 ? 0.045 : 0.0015;
                ctx.strokeStyle = darkMode ? (zoomTransform.k <= 100 ? "rgba(232, 171, 85, 0.9)" : "#aaa") : `rgba(13, 15, 11, 0.76)`;
                ctx.stroke();
            }
        }
...
function addRiverLabel(feature) {
    ...
    ctx.font = `${Math.max(1 / zoomTransform.k , 0.001)}em Arial`;
...
// Функция для получения текущей палитры
...
async function showSocialIndicators(regionName) {
    ...
    try {
        ...
        if (regionData) {
            ...
            // Создаем графики только для показателей с данными
            // Создаем графики только с доступными данными
...
async function updateInfoBoxWiki(entity, wikidataID) {
    ...
    mainContent = `
        <div style="position: relative; padding: 10px;">
            <!-- Кнопка для социальных показателей -->
            <button id="showSocialButton" style="background: ${darkMode ? '#53576e' : '#f0d3c0'}; color: ${darkMode ? 'white' : 'black'};">
                ${socialToggle ? 'Скрыть':'Показать'} показатели
            </button>
            <br>
            ${closeButton}
            <h1>${title}</h1>
    ...
    if (!socialTab) {
        ...
        <div class="social-tab-header">
          ...
          <div class="close-btn" onclick="toggleShowButton()">
...
function toggleShowButton(){
    ...
    const btn = document.getElementById('showSocialButton');
    socialToggle = !socialToggle;
    socialTab.style.display = socialToggle ? 'block' : 'none';
    btn.textContent = socialToggle ? 'Скрыть показатели' : 'Показать показатели';
}
